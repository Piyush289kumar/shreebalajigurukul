...
<section>
	<div class="container">
		...
		<div class="container">
			...
			$sql_show_user = "SELECT * FROM campus WHERE smtype = 'career' AND active_record = 'Yes' ORDER BY smid LIMIT {$offset},{$record_limit}";
		...
		<?php
                $sql_user_show_by_page = "SELECT * FROM campus WHERE smtype = 'career' AND active_record = 'Yes'";
                ...
                if (mysqli_num_rows($result_sql_user_show_by_page) > 0) {
                    ...
                    if ($page_num_index_by_addbar > 1) {
                        echo ("<li><a href='career.php?page_num_index=" . ($page_num_index_by_addbar - 1) . "'>Prev</a></li>");
                    }
                    for ($i = 1; $i <= $total_page; $i++) {
                        ...
                        echo ("<li class=$active_page><a href='career.php?page_num_index=$i'>$i</a></li>");
                    }
                    if ($page_num_index_by_addbar < $total_page) {
                        echo ("<li><a href='career.php?page_num_index=" . ($page_num_index_by_addbar + 1) . "'>Next</a></li>");
                    }
                    echo ("</ul>");
                }
                ?>
